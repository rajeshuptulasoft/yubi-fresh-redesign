...
export default function AdminLogin() {
  ...
  return <div style={page}><form onSubmit={handleLogin} style={card}><img src={yubiLogo} alt="YUBI" style={{ height: 76, objectFit: "contain", margin: "0 auto 16px", display: "block" }} /><h1 style={title}>Admin Login</h1><input type="email" placeholder="Enter admin email" value={email} onChange={(e) => setEmail(e.target.value)} style={input} /><input type="password" placeholder="Enter password" value={password} onChange={(e) => setPassword(e.target.value)} style={input} /><button style={button}>Login to Dashboard</button>{error && <p style={{ color: "#D32F2F", textAlign: "center", fontWeight: 800 }}>{error}</p>}</form></div>;
}
