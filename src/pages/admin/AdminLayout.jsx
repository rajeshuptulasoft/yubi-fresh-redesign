...
export default function AdminLayout() {
  ...
  return <div style={{ color: "#1A1A1A" }}>
    <header style={{ background: "linear-gradient(135deg,#1A2E1A,#2E7D32)", height: 68, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 22px", position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, boxShadow: "0 10px 30px rgba(26,46,26,0.2)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}><button onClick={() => setIsOpen(!isOpen)} style={hamburger}><Menu size={24} /></button><img src={yubiLogo} alt="YUBI Admin" style={{ height: 46, background: "#FFFFFF", borderRadius: 12, padding: 4 }} /><strong style={{ color: "#FFFFFF", fontSize: 18 }}>Admin</strong></div>
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}><span style={{ color: "#FFFFFF", fontSize: 14 }}>Welcome, Admin</span><button onClick={logout} style={{ background: "#EF4444", color: "#FFFFFF", padding: "8px 16px", borderRadius: 10, border: "none", cursor: "pointer", fontWeight: 800 }}>Logout</button></div>
    </header>
